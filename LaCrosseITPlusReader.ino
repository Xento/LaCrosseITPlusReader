...
//            2015-03     Schalter  0e 1e  gesetzt. FULB
//                        Datensatz Emulation HMS100-TF Sensor als Ausgabeformat.(LaCrosse.cpp modifiziert)
//                        Von der uebertragenen Temperatur dann noch den offset  von -40.0 Grad abziehen.
//                        Tested on ARDUINO NANO 328, ARDUINO IDE 1.5.6.-R2
//                        http://blog.moneybag.de/lacrosse-temperatursensor-an-arduino-nano-und-rfm12b-als-jeelink-ersatz/#more-10350

#define PROGNAME         "LaCrosseITPlusReader"
#define PROGVERS         "10.1q_HMS_mod4" 
...
static void HandleSerialPort(char c) {
  ...
  if (c == ',') {
    ...
    case 'd':
      ...
      break;
    case 'e':                            // FULB
      //HMS100TH sensor emulation mode HMS
      SetHMSMode(value);
    case 'h':
      // height
...
void setup(void) {
  ...
  SetHMSMode(1);
                                          ...
                                          // FULB
void SetHMSMode(boolean mode) {
  HMS = mode;
  LaCrosse::SetHMSMode(mode);
}
